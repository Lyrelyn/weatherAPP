...
public class SplashActivity extends NetworkActivity<ActivitySplashBinding> {
    //SplashActivity是设置的启动页，作用是在应用启动时展示启动画面，同时负责初始化界面、处理启动逻辑、加载本地城市数据等操作。
    // 通过ViewModel来管理数据，并观察数据的变化，以便在数据更新时执行相应的操作。
    // SplashActivity还负责检查应用的启动状态，包括是否是首次启动以及当天是否是首次启动，根据检查结果执行相应的逻辑。
